...
int axes(double sym[3][3],double u[3],double v[3])
	...
	double n[3],su[3],sv[3],e,f,tmp;
	//double dv, DVMAX = 0.00000000000000000000000001;
...
double RMSDshort(int n, double (*motif1)[3],double (*motif2)[3], double cutoff){
	int i,k;
	double row_norm = 0., atom_dis;
	for(i=0;i<n;++i){
		atom_dis = 0.;
		for(k=0;k<3;++k){
			atom_dis += (motif1[i][k]-motif2[i][k])*(motif1[i][k]-motif2[i][k]);
			}
		atom_dis = sqrt(atom_dis);
		if (atom_dis > cutoff)
			row_norm += (atom_dis - cutoff)*(atom_dis - cutoff);
		}
	return sqrt(row_norm/n);	
	}

double SqDisshort(int n, double (*motif1)[3],double (*motif2)[3], double cutoff){
	int i,k;
	double row_norm = 0., atom_dis;
	for(i=0;i<n;++i){
		atom_dis = 0.;
...
double BB_RMSD(int n, double (*motif1)[3],double (*motif2)[3], double cutoff){
	int i,k,p;
	double row_norm = 0., atom_dis;
	int list_exactp[10] = {1,2,3,5,6,11,12,13,15,16}; 
	int list_shortp[10] = {0,4,7,8,9,10,14,17,18,19};
	...
	for(p=0;p<10;++p)
		...
		atom_dis = sqrt(atom_dis);
		if (atom_dis > 0.05)
			row_norm += (atom_dis - 0.05)*(atom_dis - 0.05);
		}
...
double RMSDshortALL(int n, double (*motif1)[3],double (*motif2)[3], double cutoff){
	int i,k;
	double row_norm = 0., atom_dis,max_shift=0.,lambda;
	for(i=0;i<n;++i){
		atom_dis = 0.;
		for(k=0;k<3;++k){
			atom_dis += (motif1[i][k]-motif2[i][k])*(motif1[i][k]-motif2[i][k]);
		...
		row_norm += atom_dis;
		atom_dis = sqrt(atom_dis);
		if (atom_dis > max_shift)
			max_shift = atom_dis;
		}

	lambda = cutoff/max_shift;
	if (max_shift > cutoff)
		return (1.-lambda)*sqrt(row_norm/n);	
	else
		return 0.;
	}
